...
class ComfyUIWorkflow:
    ...
    def __init__(self, server_ip: str, client_id: str):
        self.server_ip = (
            server_ip.rstrip("/").replace("http://", "").replace("https://", "")
        )
    ...
    async def _process_queue(self) -> None:
        ...
        try:
            while self._queue:
                base64_images, texts, videos, extract_text, future = (
                    self._queue.popleft()
                )
                try:
                    result = await self._run_workflow(
                        base64_images, texts, videos, extract_text
                    )
    ...
    def _replace_base64_images(
        self, data: Any, base64_images: List[str]
    ) -> Tuple[Any, int]:
        ...
        def replace(d: Any) -> Any:
            if isinstance(d, dict):
                ...
                if new_data.get("class_type") == "ETN_LoadImageBase64" and counter[
                    "count"
                ] < len(base64_images):
                    if (
                        "inputs" in new_data
                        and isinstance(new_data["inputs"], dict)
                        and "image" in new_data["inputs"]
                    ):
    ...
    def _replace_video_nodes(
        self, data: Any, video_filenames: List[str]
    ) -> Tuple[Any, int]:
                ...
                if new_data.get("class_type") == "VHS_LoadVideo":
                    if (
                        ...
                        if broadcast:
                            ...
                            new_data["inputs"]["video"] = video_filenames[
                                counter["count"]
                            ]
                            counter["count"] += 1
                            inputs_modified = True
    ...
    def _count_text_nodes(self, data: Any) -> int:
        ...
        def walk(d: Any) -> None:
            nonlocal count
            if isinstance(d, dict):
                if d.get("class_type") == "Simple String" and isinstance(
                    d.get("inputs"), dict
                ):
    ...
    def _update_text_nodes(self, data: Any, texts: List[str]) -> Tuple[Any, int]:
        ...
        def replace(d: Any) -> Any:
            if isinstance(d, dict):
                new_data = dict(d)
                inputs_modified = False
                if new_data.get("class_type") == "Simple String" and counter[
                    "count"
                ] < len(merged):
    ...
    def _extract_text_outputs(
        ...
        if not showtext_nodes:
            return text_outputs
        history_entry = (
            history_data.get(prompt_id) if isinstance(history_data, dict) else None
        )
        for node_id in showtext_nodes:
            text_content = None
            if (
                history_entry
                and isinstance(history_entry, dict)
                and "prompts" in history_entry
            ):
    ...
    async def _run_workflow(
        ...
        if base64_images:
            workflow_api_modified, _ = self._replace_base64_images(
                workflow_api_modified, base64_images
            )
        if videos:
            workflow_api_modified, _ = self._replace_video_nodes(
                workflow_api_modified, videos
            )
        if texts:
            workflow_api_modified, _ = self._update_text_nodes(
                workflow_api_modified, texts
            )
        ...
        return await self._wait_and_collect_result(
            prompt_id, workflow_api_modified, extract_text
        )
    ...
    async def submit_only(
        ...
        if base64_images:
            workflow_api_modified, img_count = self._replace_base64_images(
                workflow_api_modified, base64_images
            )
            if debug:
                logger.info(
                    "[ComfyUI Debug] Replaced %d ETN_LoadImageBase64 node(s) with %d image(s)",
                    img_count,
                    len(base64_images),
                )
        if videos:
        ...
        if texts:
            workflow_api_modified, replaced = self._update_text_nodes(
                workflow_api_modified, texts
            )
            if debug:
                logger.info(
        ...
        if debug:
            try:
                payload = json.dumps(
                    workflow_api_modified, ensure_ascii=False, indent=2
                )
                logger.info(
                    "[ComfyUI Debug] Full workflow JSON sent to ComfyUI (first 50k chars):\n%s",
                    payload[:50000],
                )
                if len(payload) > 50000:
                    logger.info(
                        "[ComfyUI Debug] ... (truncated, total %d chars)", len(payload)
                    )
            except Exception as e:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code >= 400:
                try:
                    ...
                    if body:
                        logger.warning(
                            "[ComfyUI] /prompt %s response body: %s",
                            resp.status_code,
                            body[:2000],
                        )
                except Exception:
    ...
    async def _wait_and_collect_result(
        ...
        text_outputs = []
        if extract_text:
            text_outputs = self._extract_text_outputs(
                workflow_api_modified, image_info, prompt_id
            )
            text_outputs = [
                t.split("</think>", 1)[-1].strip() for t in text_outputs if t.strip()
            ]
            ...
            if isinstance(out, dict) and "audio" in out:
                for audio in out["audio"]:
                    if audio.get("type") == "output":
                        ...
                        url = (
                            f"{self._base}/view?filename={fn}&subfolder={sub}&type=output"
                            if sub
                            else f"{self._base}/view?filename={fn}&type=output"
                        )
