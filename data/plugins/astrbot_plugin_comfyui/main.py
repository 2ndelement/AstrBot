...
# 每个任务预估耗时（秒），用于等待策略
ESTIMATE_SECONDS_PER_JOB = 15
WAIT_THRESHOLD_SECONDS = 10
...
def _get_wait_threshold(config: Any) -> int:
    ...
    raw = (
        getattr(config, "wait_threshold_seconds", None)
        if not isinstance(config, dict)
        else config.get("wait_threshold_seconds")
    )
    if raw is None:
...
async def _save_image_to_persistent_path(
    temp_path: str, session_key: str
) -> Optional[str]:
    """
    ...
    try:
        ...
        safe_key = "".join(
            ...
            for c in (session_key or "default")[:32]
        )
...
async def _save_video_to_persistent_path(
    temp_path: str, session_key: str
) -> Optional[str]:
...
async def _download_media_to_temp(
    media_url: str, suffix: str = ".mp4", timeout: float = 120.0
) -> Optional[str]:
    """
...
async def _send_image_to_session(
    session_id: str, image_url: str, plain_text: Optional[str] = None
) -> bool:
    """
    向指定会话发送图片（可选带一句文本）。
...
async def _send_local_image_to_session(session_id: str, image_path: str) -> bool:
    """向指定会话单独发送一条仅含图片的消息（从本地文件路径）。"""
    if not session_id or not image_path or not Path(image_path).exists():
        return False
    ctx = _plugin_context
    if not ctx:
        return False
    try:
        ...
        try:
            ...
            from astrbot.core.message.message_event_result import MessageEventResult
        try:
            result = MessageEventResult().image_result(image_path)
        except (AttributeError, TypeError):
            try:
                seg = Image.fromFileSystem(image_path)
            except AttributeError:
                seg = Image.from_file_system(image_path)
            try:
        ...
        logger.warning("ComfyUI send local image to session failed: %s", e)
        return False
...
async def _send_video_to_session(session_id: str, video_path: str) -> bool:
    ...
    try:
        try:
            ...
            logger.warning(
                "ComfyUI: Video component not available, skip sending video."
            )
            return False
        try:
            from astrbot.api.event import MessageEventResult
        ...
        if (
            ...
            and not _is_persistent_media_path(video_path)
        ):
            try:
...
async def _get_queue_status(server_ip: str) -> tuple:
    ...
    base = (
        f"http://{server_ip.lstrip('/').replace('http://', '').replace('https://', '')}"
    )
    try:
        ...
        if isinstance(out, dict) and "audio" in out:
            for audio in out["audio"]:
                if audio.get("type") == "output":
                    ...
                    url = (
                        f"{base}/view?filename={fn}&subfolder={sub}&type=output"
                        if sub
                        else f"{base}/view?filename={fn}&type=output"
                    )
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for comp in chain:
                comp_type = getattr(comp, "type", None) or (
                    comp.get("type") if isinstance(comp, dict) else None
                )
                if comp_type in ("image", "Image"):
                    url = getattr(comp, "url", None) or (
                        comp.get("url") if isinstance(comp, dict) else None
                    )
                    if not url:
                        file_path = getattr(comp, "file", None) or (
                            comp.get("file") if isinstance(comp, dict) else None
                        )
        ...
        for err in err_list:
            ...
            if err.get("type") == "value_not_in_list":
                ...
                if (
                    ...
                    and len(config_list)
                    and isinstance(config_list[0], list)
                ):
                ...
                if input_name == "ckpt_name" and received:
                    allowed_str = (
                        "、".join(allowed) if allowed else "(见服务器模型目录)"
                    )
                    parts.append(
...
def _sanitize_image_urls_for_log(image_urls: Any) -> str:
    ...
    if isinstance(image_urls, str):
        return (
            "<base64 or long string>"
            if _looks_like_base64(image_urls)
            else (image_urls[:80] + "..." if len(image_urls) > 80 else image_urls)
        )
        ...
        for u in image_urls[:10]:
            if isinstance(u, str):
                if u.startswith("http"):
                    ...
                    _looks_like_base64(u)
                    or u.startswith("data:image")
                    or u.startswith("base64:")
                ):
                    parts.append("<base64>")
                else:
    ...
    for s in sources:
        ...
        if s.startswith("data:image") and "base64," in s:
            ...
            if b64:
                result.append(b64)
                logger.info(
                    "[ComfyUI Tool] Using image from data URI (base64) in image_urls."
                )
            continue
        ...
        if raw:
            try:
                ...
                result.append(raw)
                logger.info(
                    "[ComfyUI Tool] Using image from base64: prefix in image_urls."
                )
            except Exception as e:
        ...
        if len(s) >= 100 and all(
            ...
            try:
                ...
                result.append(clean)
                logger.info(
                    "[ComfyUI Tool] Using image from raw base64 string in image_urls."
                )
                continue
            except Exception:
                pass
        ...
        if not s.startswith("http"):
            ...
            if not _is_allowed_local_image_path(p):
                logger.warning(
                    "ComfyUI rejected local image path (outside allowed dir): %s",
                    s[:80],
                )
                continue
            try:
...
class ComfyUIListWorkflowsTool(FunctionTool[AstrAgentContext]):
    ...
    async def call(
        ...
        logger.info(
            "[ComfyUI Tool] comfyui_list_workflows called with args: %s", kwargs
        )
        ...
        for w in workflows:
            ...
            if i > 0:
                req_parts.append(
                    f"{i} image(s) (required: from user message, or pass image_urls: URL or local path, e.g. from get_image_from_context)"
                )
            if v > 0:
...
class ComfyUIStatusTool(FunctionTool[AstrAgentContext]):
    ...
    async def call(
        ...
        if pending and pending.get("prompt_id") and server_ip:
            remaining = await _estimate_remaining_seconds(
                server_ip, pending["prompt_id"]
            )
            if remaining == 0:
                url, ftype, texts = await _get_result_for_prompt(
                    server_ip, pending["prompt_id"]
                )
                ...
                if url:
                    if ftype == "image":
                        if _is_local_image_url(url, server_ip):
                            _session_image_url_queue.setdefault(session_key, []).append(
                                url
                            )
                            return (
                                "Task completed. Output: image. The image will be sent as a separate message automatically. Just reply normally. Queue: 0 running, 0 pending. "
                                f"IMAGE_PATH: {url} [COMFYUI_IMAGE]"
                            )
                        session_id = _get_session_id_from_context(context.context)
                        if session_id:
                            sent = await _send_image_to_session(session_id, url, "图好了～")
                            if sent:
                                return f"Task completed. Output: image. Image has been sent to the user. IMAGE_URL: {url} Queue: 0 running, 0 pending."
                        return f"Task completed. Output: image. IMAGE_URL: {url} Queue: 0 running, 0 pending."
                    if ftype == "video":
            ...
            remaining_after = await _estimate_remaining_seconds(
                server_ip, pending["prompt_id"]
            )
            return (
    ...
    async def call(
        ...
        if task_id_arg:
            ...
            if pending:
                ...
                logger.info(
                    "[ComfyUI Tool] Using task from registry: task_id=%s", task_id_arg
                )
        if not pending:
            pending = _session_pending.get(session_key) or _session_pending.get(
                "default"
            )
        if not pending:
            ...
            if running_n >= 0 and (running_n + pending_n) == 1:
                ...
                if first:
                    ...
                    logger.info(
                        "[ComfyUI Tool] Recovered pending from queue for session (1 task in queue): %s",
                        prompt_id_first,
                    )
            if not pending:
            ...
            if url:
                if ftype == "image":
                    if _is_local_image_url(url, server_ip):
                        ...
                        return (
                            "Task completed. Output: image. The image will be sent as a separate message automatically. Just reply normally. "
                            f"IMAGE_PATH: {url}{extra} [COMFYUI_IMAGE]"
                        )
                    session_id = _get_session_id_from_context(context.context)
                    sent = False
                    if session_id:
                ...
                if ftype == "image":
                    ...
                    return f"Task completed. Output: image. IMAGE_URL: {url}{extra}"
                if ftype == "video":
                    _session_video_url_queue.setdefault(session_key, []).append(url)
                    return (
                        f"Task completed. Output: video. Do NOT call send_message_to_user for this video (it will become voice). "
        ...
        if remaining < wait_threshold:
            client_id = pending.get("client_id", "")
            url, ftype, texts = await _wait_for_completion(
                server_ip, client_id, prompt_id, timeout=remaining + 120
            )
            for k in list(_session_pending.keys()):
                if _session_pending.get(k) == pending:
                    _session_pending.pop(k, None)
...
class ComfyUIExecuteTool(FunctionTool[AstrAgentContext]):
    ...
    async def call(
        ...
        if image_urls_arg:
            ...
            if from_sources:
                logger.info(
                    "[ComfyUI Tool] Injected %d image(s) from image_urls placeholder (URL or local path).",
                    len(from_sources),
                )
        ...
        if (
            len(images_b64) < info["images"]
            or len(texts) < info["texts"]
            or len(videos) < info.get("videos", 0)
        ):
            hint = ""
            if len(images_b64) < info["images"] and len(images_b64) == 0:
                hint = " No image in current message. Use image_urls: HTTP URL, or local path under plugin data dir or data/agent/comfyui/input (absolute path)."
            return (
        ...
        try:
            ...
            prompt_id = await workflow.submit_only(
                images_b64, texts, videos, debug=debug
            )
            session_key = _get_session_key(context.context)
            pending_data = {
                "prompt_id": prompt_id,
            ...
            msg = (
                ...
                + (
                    summary
                    if summary
                    else (f"Server message: {body[:1500]}" if body else str(e))
                )
            )
            ...
            return (
                msg
                + (
                    " Suggest: fix workflow or use another workflow that runs on this server."
                    if summary
                    else " Possible causes: workflow node/input mismatch, invalid image format, or server error."
                )
                + desc_reminder
            )
        except Exception as e:
...
class ComfyUIPlugin(Star):
    ...
    async def on_decorating_result(self, event: AstrMessageEvent) -> None:
        ...
        # 取本会话图片 URL（FIFO）
        iq = _session_image_url_queue.get(session_key) or _session_image_url_queue.get(
            "default"
        )
        image_url = iq.pop(0) if (iq and len(iq) > 0) else None
        ik = (
            session_key
            if (session_key and _session_image_url_queue.get(session_key))
            else "default"
        )
        if iq is not None and len(iq) == 0:
            _session_image_url_queue.pop(ik, None)
        # 取本会话视频 URL（FIFO）
        vq = _session_video_url_queue.get(session_key) or _session_video_url_queue.get(
            "default"
        )
        video_url = vq.pop(0) if (vq and len(vq) > 0) else None
        vk = (
            session_key
            if (session_key and _session_video_url_queue.get(session_key))
            else "default"
        )
        ...
        if image_url and (not temp_path or not Path(temp_path).exists()):
            temp_path = None
        video_temp_path = (
            await _download_media_to_temp(video_url, ".mp4") if video_url else None
        )
        ...
        if temp_path:
            persistent_image_path = await _save_image_to_persistent_path(
                temp_path, session_key or ""
            )
            if persistent_image_path:
                image_path_for_send = persistent_image_path
        # 从所有文本段中移除占位符；图片单独发一条消息，不混入文本 chain
        for seg in chain:
            text = getattr(seg, "text", None) if seg is not None else None
            if text is not None and COMFYUI_IMAGE_PLACEHOLDER in (
                text if isinstance(text, str) else ""
            ):
                cleaned = (text or "").replace(COMFYUI_IMAGE_PLACEHOLDER, "").strip()
                if cleaned:
                    new_chain.append(Plain(cleaned))
            else:
                new_chain.append(seg)
        if image_path_for_send:
            _img_session_id = getattr(event, "unified_msg_origin", None) or ""
            if not _img_session_id and hasattr(event, "get_session_id"):
                _img_session_id = str(event.get_session_id() or "")
            if _img_session_id and ":" in _img_session_id:
                _isid = _img_session_id
                _ipath = image_path_for_send

                async def _send_image_later() -> None:
                    await asyncio.sleep(0.3)
                    await _send_local_image_to_session(_isid, _ipath)

                asyncio.create_task(_send_image_later())
        # 视频不与文本混在同一条消息：另存到持久化路径，消息中带出路径，再单独发一条视频
        video_path_for_send = video_temp_path
        persistent_video_path: Optional[str] = None
        if video_temp_path:
            persistent_video_path = await _save_video_to_persistent_path(
                video_temp_path, session_key or ""
            )
        ...
        if video_temp_path:
            ...
            for seg in new_chain:
                text = getattr(seg, "text", None) if seg is not None else None
                if text is not None and COMFYUI_VIDEO_PLACEHOLDER in (
                    text if isinstance(text, str) else ""
                ):
                    new_text = (
                        (text or "").replace(COMFYUI_VIDEO_PLACEHOLDER, "").strip()
                    )
                    if new_text:
    ...
    async def cmd_comfyui(self, event: AstrMessageEvent):
        ...
        if msg == "查询" or msg == "list" or msg == "help":
            ...
            if not workflows:
                yield event.plain_result(
                    "当前没有工作流文件。请使用 /comfyui 上传 并回复一条包含 .json 文件的消息。"
                )
                return
            lines = ["工作流列表："]
            for w in workflows:
        ...
        if msg == "上传" or msg == "upload":
            ...
            if reply:
                reply_chain = (
                    getattr(getattr(reply, "message_obj", None), "message", None)
            ...
            file_name = None
            for comp in chain:
                ctype = getattr(comp, "type", None) or (
                    comp.get("type") if isinstance(comp, dict) else None
                )
                if ctype in ("file", "File", "image", "Image"):
                    ...
                    name = (
                        ...
                        or (
                            comp.get("name") or comp.get("filename")
            ...
            if not file_url:
                yield event.plain_result(
                    "请回复一条包含 .json 工作流文件的消息，然后发送 /comfyui 上传。"
                )
                return
            _ensure_workflows_dir()
            try:
            ...
            return
        yield event.plain_result(
            "用法：/comfyui 查询 | /comfyui 上传（需回复含 .json 的消息）"
        )
