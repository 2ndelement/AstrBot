...
async def call_onebot(client, action: str, **kwargs) -> Any:
    ...
    if hasattr(client, "call_action") and callable(
...
def parse_leaked_tool_call(
    text: str, filter_patterns: List[str] = None
...
async def check_tool_permission(
    ...
    # 2. 检查工具是否在受限列表中
    admin_only_tools = permission_config.get(
        "admin_only_tools",
        [
            "ban_user",
            "group_ban",
            ...
            if isinstance(event, AiocqhttpMessageEvent) and event.get_group_id():
                if client:
                    try:
                        ...
                        logger.warning(
                            f"Failed to check group role for permission: {e}"
                        )
        except ImportError:
            pass
    ...
    return (
        False,
        f"权限不足：工具 {tool_name} 仅限管理员或白名单用户使用。你的用户ID: {sender_id}，如需使用请联系管理员。",
    )
