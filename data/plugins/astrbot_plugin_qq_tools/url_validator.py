...
class URLValidator:
    ...
    PRIVATE_IPV4_NETWORKS = [
        ...
        ipaddress.ip_network(
            "169.254.0.0/16"
        ),  # Link-Local (包含 AWS 元数据 169.254.169.254)
    ...
    def __init__(
        ...
        self._allowed_patterns = [
            self._compile_domain_pattern(d) for d in self.allowed_domains
        ]
        self._blocked_patterns = [
            self._compile_domain_pattern(d) for d in self.blocked_domains
        ]
        ...
        if isinstance(ip, ipaddress.IPv4Address):
            for network in self.PRIVATE_IPV4_NETWORKS:
                if ip in network:
                    return (
                        True,
                        f"拒绝访问私有/保留 IPv4 地址: {ip_str} (属于 {network})",
                    )
            return False, ""
        ...
        if isinstance(ip, ipaddress.IPv6Address):
            ...
            for network in self.PRIVATE_IPV6_NETWORKS:
                if ip in network:
                    return (
                        True,
                        f"拒绝访问私有/保留 IPv6 地址: {ip_str} (属于 {network})",
                    )
            return False, ""
