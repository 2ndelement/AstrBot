...
PLATFORM_EVENT_REGISTRY: Dict[str, Tuple[str, str, Optional[callable]]] = {
    ...
    "slack": (
        ...
        lambda platform: {
            "web_client": getattr(
                platform, "web_client", getattr(platform, "client", None)
            )
        },
    ),
    # 飞书 (Lark)
    "lark": (
        ...
        "LarkMessageEvent",
        lambda platform: {
            "bot": getattr(platform, "bot", getattr(platform, "lark_client", None))
        },
    ),
    # 钉钉
    "dingtalk": (
        ...
        lambda platform: {
            "client": getattr(platform, "client", getattr(platform, "handler", None))
        },
    ),
    # 企业微信
    "wecom": (
...
def get_platform_event_class(
    platform_name: str,
) -> Optional[Tuple[Type[AstrMessageEvent], Optional[callable]]]:
    """动态获取平台对应的事件类
    ...
    module_path, class_name, extra_kwargs_builder = PLATFORM_EVENT_REGISTRY[
        platform_name
    ]
...
class QQToolsPlugin(Star):
    def __init__(self, context: Context, config: Dict):
        ...
        # 缓存清理配置
        self.cache_inactive_timeout = self.general_config.get(
            "cache_inactive_timeout", 3600
        )  # 默认 1 小时
        self.cache_cleanup_interval = self.general_config.get(
        ...
        logger.info(
            f"QQToolsPlugin loaded. Cache size: {self.cache_size}, inactive timeout: {self.cache_inactive_timeout}s."
        )
        ...
        self.cache_cleanup_task = asyncio.create_task(
            self._cleanup_inactive_caches_loop()
        )
    ...
    def _manage_browser_tools(self):
        ...
        original_browser_tool_names = [
            "browser_open",
            "browser_click",
            "browser_grid_overlay",
            "browser_click_relative",  # 新增
            "browser_click_in_element",
            "browser_input",
            "browser_scroll",
            "browser_get_link",
            "browser_view_image",
        ...
        if self.add_tool_prefix:
            current_names = [
                f"{self.tool_prefix}{name}" for name in original_browser_tool_names
            ]
            legacy_names = original_browser_tool_names  # 无前缀版本是残余
        else:
            current_names = original_browser_tool_names
            legacy_names = [
                f"qts_{name}" for name in original_browser_tool_names
            ]  # 带前缀版本是残余
        ...
        if self.tool_config.get("browser", False):
            ...
            if self.general_config.get("auto_install_browser_deps", False):
                ...
                asyncio.create_task(
                    self._async_install_browser_deps_and_register(legacy_names)
                )
        ...
        try:
            ...
            if installed:
                ...
                try:
                    ...
                    logger.info(
                        "Browser modules reloaded after dependency installation."
                    )
    ...
    def _manage_wake_tools(self):
        ...
        if self.add_tool_prefix:
            current_names = [
                f"{self.tool_prefix}{name}" for name in original_tool_names
            ]
    ...
    async def _wake_callback(self, task: WakeTask):
        ...
        try:
            ...
            # 获取 bot 自身 ID
            bot_self_id = (
                str(platform.client_self_id)
                if hasattr(platform, "client_self_id")
                else "0"
            )
            ...
            if event is None:
                ...
                await self.context.send_message(
                    task.session_id, MessageEventResult().message(wake_message)
                )
                return
            ...
            logger.info(
                f"Wake event committed for session {task.session_id} (platform: {platform_name})"
            )
        ...
        try:
            ...
            if extra_kwargs_builder is not None:
                try:
                    ...
                    logger.debug(
                        f"Failed to build extra kwargs for {platform_name}: {e}"
                    )
    ...
    async def _ensure_browser_deps_async(self) -> bool:
        ...
        for i in range(max_retries):
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    logger.warning(
                        f"Failed to install playwright (Attempt {i + 1}): {stderr.decode()}"
                    )
            except Exception as e:
        ...
        for i in range(max_retries):
            ...
            if i > 0:  # 重试时设置镜像环境变量
                ...
                logger.info(
                    f"Installing chromium with mirror (Attempt {i + 1}/{max_retries})..."
                )
            else:
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    logger.warning(
                        f"Failed to install chromium (Attempt {i + 1}): {stderr.decode()}"
                    )
            except Exception as e:
    ...
    async def _cleanup_inactive_caches(self):
        ...
        inactive_sessions = [
            sid
            for sid, last_active in self.cache_last_active.items()
        ...
        if inactive_sessions:
            logger.debug(
                f"Cleaned up {len(inactive_sessions)} inactive session caches."
            )
            ...
            if AiocqhttpMessageEvent and isinstance(event, AiocqhttpMessageEvent):
                ...
                if raw_dict:
                    ...
                    if post_type == "notice":
                        ...
                        # 检查是否是 poke notice
                        if (
                            raw_dict.get("notice_type") == "notify"
                            ...
                            poke_info = {
                                ...
                                "group_id": raw_dict.get(
                                    "group_id"
                                ),  # 群号（私聊时无）
                                "raw_info": raw_dict.get("raw_info"),  # 动作文案信息
                                "raw_message": raw_dict.get(
                                    "raw_message"
                                ),  # 兼容旧字段
                            ...
                            logger.debug(
                                f"Cached poke notice: user_id={poke_info['user_id']}, target_id={poke_info['target_id']}, raw_info={poke_info['raw_info']}"
                            )
        except Exception as e:
        ...
        try:
            ...
            if file_info_parts:
                ...
                if file_info_parts[0] not in event.message_str:
                    ...
                    if hasattr(event.message_obj, "message_str") and isinstance(
    ...
    def _extract_file_info(self, event: AstrMessageEvent) -> list:
        ...
        if (
            raw_message
            and hasattr(raw_message, "message")
            and isinstance(raw_message.message, list)
        ):
        ...
        for comp in messages:
            ...
            if isinstance(comp, Comp.File):
                ...
                # 视频类型
                file_name = (
                    getattr(comp, "path", "") or getattr(comp, "file", "") or "video"
                )
                if "/" in file_name:
                ...
                # 音频类型
                file_name = (
                    getattr(comp, "path", "") or getattr(comp, "file", "") or "audio"
                )
                if "/" in file_name:
                    file_name = file_name.split("/")[-1]
                if "\\" in file_name:
                ...
                if "record" in raw_data_by_type and idx < len(
                    raw_data_by_type["record"]
                ):
                ...
                if self.general_config.get("show_image_as_file", False):
                    ...
                    if "image" in raw_data_by_type and idx < len(
                        raw_data_by_type["image"]
                    ):
                        ...
                        if "file_id" in raw_data:
                            ...
                            if (
                                file_val
                                and not file_val.startswith("http")
                                and not file_val.startswith("base64")
                            ):
                                info_parts.append(f"id={file_val[:16]}")
                        if "file_size" in raw_data:
                            info_parts.append(f"size={raw_data['file_size']}")
    ...
    def _enhance_reply_quote(self, event: AstrMessageEvent):
        ...
        include_msg_id = bool(
            self.general_config.get("reply_quote_include_msg_id", True)
        )
        inject_into_message_str = bool(
            self.general_config.get("inject_reply_quote_into_message_str", True)
        )
        enrich_even_if_text = bool(
            self.general_config.get("reply_quote_enrich_even_if_text", True)
        )
        ...
        for seg in msgs:
            ...
            if include_msg_id:
                ...
                if mid and mid not in summary:
                    ...
                    summary_with_id = (
                        f"{summary} MSG_ID:{mid}" if summary else f"MSG_ID:{mid}"
                    )
            ...
            if hasattr(event.message_obj, "message_str") and isinstance(
                ...
                if not event.message_obj.message_str.startswith(prefix.strip()):
                    event.message_obj.message_str = (
                        prefix + event.message_obj.message_str
                    )
    ...
    def _parse_bilibili_from_payload(
        self, payload: str, source_type: str
    ) -> Optional[Dict]:
        ...
        return {"bvid": bvid, "aid": aid, "url": url, "p": p}
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        ...
        enable_at_conversion = self.general_config.get(
            "enable_auto_at_conversion", False
        )
        msg_filter_patterns = self.general_config.get("message_filter_patterns", [])

        # 如果没有任何功能启用，直接返回
        if (
            not enable_reply_adapter
            and not enable_at_conversion
            and not msg_filter_patterns
        ):
            return
            ...
            if isinstance(component, Comp.Plain) and component.text:
                ...
                if enable_reply_adapter and has_protocol_b_markers(current_text):
                    ...
                    if self.compatibility_config.get("fix_tool_leak", True):
                        filter_patterns = self.compatibility_config.get(
                            "filter_patterns", ["&&.*?&&"]
                        )
                        content, message_id = parse_leaked_tool_call(
                            current_text, filter_patterns=filter_patterns
                        )

                        if content is not None and message_id is not None:
                            # 解析成功，构造 Reply 和 Content
                            logger.warning(
                                f"Detected leaked tool call in text. Fixing... ID: {message_id}, Content: {content}"
                            )
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req):
        ...
        try:
            ...
            if hasattr(req, "system_prompt"):
                ...
                logger.warning(
                    "ProviderRequest has no 'system_prompt' attribute, cannot inject prompt."
                )
    ...
    async def fetch_history_from_api(
        self, event: AstrMessageEvent, count: int = 50
    ) -> list:
        ...
        try:
            if event.get_group_id():
                group_id = int(event.get_group_id())
                resp = await call_onebot(
                    client, "get_group_msg_history", group_id=group_id, count=count
                )
            else:
                user_id = int(event.get_sender_id())
                resp = await call_onebot(
                    client, "get_friend_msg_history", user_id=user_id, count=count
                )
