...
async def _check_browser_tool_permission(
    plugin, tool_name: str, event
    ...
    return await check_tool_permission(original_name, event, permission_config, client)


def _replace_image_in_content(
    content: list, image_id: str, new_part: ImageURLPart
) -> list:
    ...
    try:
        ...
        if messages:
            ...
            img_part = ImageURLPart(
                image_url=ImageURLPart.ImageURL(url=data_url, id=image_id)
            )
            ...
            for msg in reversed(messages):
                if msg.role == "user":
                    ...
                    if isinstance(msg.content, list):
                        # 使用替换逻辑，移除旧的同 ID 图片
                        msg.content = _replace_image_in_content(
                            msg.content, image_id, img_part
                        )
                        logger.info(
                            f"Image injected to LLM context with id='{image_id}' (replaced old one)."
                        )
                        break
...
class BrowserOpenTool(FunctionTool):
    ...
    async def call(
        ...
        # 工具权限检查
        has_permission, reason = await _check_browser_tool_permission(
            self.plugin, self.name, event
        )
        ...
        return await inject_browser_image(
            context, screenshot, info, success_suffix=detailed_suffix
        )
...
class BrowserGridOverlayTool(FunctionTool):
    ...
    async def call(
        ...
        # 获取网格截图
        screenshot, info = await browser_manager.get_grid_overlay_screenshot(
            float(grid_step)
        )
...
class BrowserWaitTool(FunctionTool):
    ...
    async def call(
        ...
        try:
            ...
            # 尝试等待网络空闲（最多再等2秒）
            try:
                await browser_manager.page.wait_for_load_state(
            ...
            # 注入截图到上下文（使用共享函数）
            return await inject_browser_image(
                context, screenshot, f"已等待 {seconds} 秒，页面内容已更新。{info}"
            )
...
class BrowserSendImageTool(FunctionTool):
    ...
    async def _get_image_url_from_element(
        self, element_id: int
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if resp.status != 200:
                        logger.warning(
                            f"Failed to download image: HTTP {resp.status} - {url}"
                        )
                        return None
                    ...
                    if not content_type.startswith("image/"):
                        logger.warning(
                            f"Not an image content type: {content_type} - {url}"
                        )
                    ...
                    if content_length and int(content_length) > 50 * 1024 * 1024:
                        logger.warning(
                            f"Image too large: {content_length} bytes - {url}"
                        )
                        return None
...
class BrowserClickInElementTool(FunctionTool):
    ...
    async def call(
        ...
        # 在元素内点击
        screenshot, info = await browser_manager.click_in_element(
            int(element_id), float(rx), float(ry)
        )
