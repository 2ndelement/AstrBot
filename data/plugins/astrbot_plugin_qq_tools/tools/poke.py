...
class PokeTool(FunctionTool):
    ...
    async def call(
        ...
        try:
            ...
            if is_group:
                # 群聊戳一戳
                await call_onebot(
                    client, "group_poke", group_id=int(group_id), user_id=int(qq_id)
                )
            else:
                # 私聊戳一戳
    ...
    async def _wait_for_poke_notice(
        ...
        if not self.plugin:
            logger.warning(
                "PokeTool: plugin instance not available, cannot get poke notice"
            )
            return None
    ...
    def _find_matching_poke_notice(
        self, self_id: str, target_id: str, group_id: Optional[str], start_ts: float
    ) -> Optional[dict]:
    ...
    async def _get_target_name(
        self, client, target_id: str, group_id: Optional[str]
    ) -> str:
        """获取目标用户的显示名称
        ...
        try:
            if group_id:
                ...
                try:
                    info = await call_onebot(
                        client, "get_friend_info", user_id=int(target_id)
                    )
                    if info:
