...
class GroupMuteAllTool(FunctionTool):
    ...
    async def call(
        ...
        try:
            ...
            bot_member_info = await call_onebot(
                client,
            ...
            if enable_param is None:
                # 获取群信息，查看全体禁言状态
                group_info = await call_onebot(
                    client, "get_group_info", group_id=group_id, no_cache=True
                )
            ...
            # 5. 执行全体禁言设置
            await call_onebot(
                client, "set_group_whole_ban", group_id=group_id, enable=enable
            )
