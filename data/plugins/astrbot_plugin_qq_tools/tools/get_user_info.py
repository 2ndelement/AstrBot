...
class GetUserInfoTool(FunctionTool):
    ...
    async def call(
        ...
        if is_group:
            try:
                honor_info = await call_onebot(
                    client, "get_group_honor_info", group_id=group_id, type="talkative"
                )
                if honor_info and "current_talkative" in honor_info:
                    dragon_king_uin = str(
                        honor_info["current_talkative"].get("user_id", "")
                    )
        ...
        for uid in target_ids:
            ...
            try:
                if is_group:
                    ...
                    details = []
                    if sex != "unknown":
                        details.append(f"性别: {sex}")
                    if age != "未知":
                        details.append(f"年龄: {age}")
                    if area:
                        details.append(f"地区: {area}")
                    ...
                    # 私聊场景
                    stranger_info = await call_onebot(
                        client, "get_stranger_info", user_id=int(uid)
                    )
