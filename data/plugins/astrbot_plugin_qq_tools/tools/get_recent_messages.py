...
class GetRecentMessagesTool(FunctionTool):
    def __init__(self, plugin):
        ...
        desc = "搜索当前会话的历史消息记录。【仅在用户明确要求查找历史消息、或需要定位特定 message_id 时调用】。不要在对话开始时主动调用——当前消息已完整传入上下文。"
        if not show_message_id:
            desc = "搜索当前会话的历史消息记录。【仅在用户明确要求查找历史消息、或需要定位特定 message_id（用于引用/撤回/转发）时调用】。不要在对话开始时主动调用——当前消息已完整传入上下文。"
    ...
    async def call(
        ...
        if include_bot_messages and isinstance(event, AiocqhttpMessageEvent):
            try:
                api_messages = await self.plugin.fetch_history_from_api(
                    event, count * 2
                )
        ...
        logger.info(
            f"Searching messages in session {session_id}. Total: {len(messages)} (cached: {len(cached_messages)}, api: {len(api_messages)}). Filter: sender={sender_filter}, sender_id={sender_id_filter}, keyword={keyword}, time={start_time}-{end_time}"
        )
            ...
            if sender_filter or sender_id_filter or keyword or start_time or end_time:
                ...
                for msg in messages[:5]:
                    ...
                    fallback_result.append(
                        f"[{time_str}] {msg['sender_name']}{is_bot}({sender_qq}) (ID: {msg['message_id']}): {msg['content'][:50]}"
                    )

                hint = ""
                if sender_id_filter == self_id:
                    hint = (
                        "\n提示：您正在查找 BOT 发送的消息。BOT 消息的 sender_id 是 "
                        + self_id
                    )

                return (
                    f"未找到完全匹配的消息。以下是最近的 5 条消息，请检查筛选条件是否正确：{hint}\n"
        ...
        for msg in matched_msgs:
            ...
            if use_detail_preview:
                ...
                # 标记 BOT 消息
                is_bot_msg = (
                    msg.get("is_bot_message", False) or str(sender_qq) == self_id
                )
                bot_marker = " [BOT]" if is_bot_msg else ""

                result.append(
                    f"[{time_str}] {msg['sender_name']}{bot_marker}({sender_qq}) (ID: {msg['message_id']}): {preview}"
                )
            else:
                # 原有模式 (数量>50)
                sender_qq = msg.get("sender_id", "Unknown")
                ...
                bot_marker = " [BOT]" if is_bot_msg else ""
                result.append(
                    f"[{time_str}] {msg['sender_name']}{bot_marker} (ID: {msg['message_id']}): {msg['content']}"
                )
        ...
        bot_count = sum(
            ...
            if m.get("is_bot_message", False) or str(m.get("sender_id", "")) == self_id
        )
