...
class DeleteMessageTool(FunctionTool):
    def __init__(self, plugin):
        ...
        super().__init__(
            ...
            parameters={
                "type": "object",
                "properties": {
                    "message_id": {"type": "string", "description": desc_message_id}
                },
                "required": ["message_id"],
            },
        )
    ...
    async def call(
        ...
        if self.plugin:
            ...
            original_name = get_original_tool_name(
                self.name, self.plugin.add_tool_prefix
            )
        ...
        for mid in ids:
            ...
            if not msg_info:
                ...
                try:
                    ...
                    results.append(
                        f"消息 {mid}: 撤回失败 (无法获取消息详情(可能已过期)且直接撤回失败: {e})"
                    )
                continue
            ...
            # --- Group Chat ---
                ...
                if my_role == "member":
                    if sender_id != self_id:
                        results.append(
                            f"消息 {mid}: 撤回失败 (权限不足，对方为{target_role_cn}，你为{my_role_cn})"
                        )
                        continue
                    if is_timeout:
                        results.append(
                            f"消息 {mid}: 撤回失败 (时间超过两分钟，且你不是管理员)"
                        )
                        continue
                ...
                # 2. Admin
                    ...
                    if sender_id == self_id:
                        ...
                        results.append(
                            f"消息 {mid}: 撤回失败 (权限不足，对方为群主，你为管理员)"
                        )
                        continue
                    elif target_role == "admin":
                        if sender_id != self_id:
                            results.append(
                                f"消息 {mid}: 撤回失败 (权限不足，对方为管理员，你为管理员)"
                            )
                            continue
                    else:
