...
class ViewAvatarTool(FunctionTool):
    ...
    async def _inject_to_context(
        self, context: ContextWrapper[AstrAgentContext], user_id: str, avatar_url: str
    ) -> str:
        """将头像图片注入到 LLM 上下文中"""
        try:
            ...
            if messages:
                ...
                img_part = ImageURLPart(
                    image_url=ImageURLPart.ImageURL(
                        url=avatar_url, id=f"avatar_{user_id}"
                    )
                )
