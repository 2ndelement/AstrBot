...
class ViewVideoTool(FunctionTool):
    ...
    async def call(
        ...
        has_permission, reason = await check_tool_permission(
            original_name, event, permission_config, client
        )
        ...
        if (
            not message_id
            and not bilibili_input
            and element_id is None
            and not video_url_input
        ):
            return (
                "❌ 缺少参数：请填写以下参数之一：\n"
                "• message_id - QQ视频消息ID\n"
        ...
        try:
            if message_id:
                ...
                try:
                    # 尝试获取消息详情
                    try:
                        msg_data = await call_onebot(
                            client, "get_msg", message_id=int(message_id)
                        )
                        ...
                        return self._format_error(
                            "获取消息详情",
                            e,
                            f"调用 get_msg API 失败，message_id={message_id}",
                        )
                        ...
                        for comp in message_content:
                            if isinstance(comp, dict):
                                ...
                                if comp_type == "file":
                                    ...
                                    if f_name and f_name.lower().endswith(
                                        video_extensions
                                    ):
                                        video_comp = comp
                                        video_comp["_is_file_video"] = True
                                        logger.info(
                                            f"Detected video file sent as file type: {f_name}"
                                        )
                                        break
                    ...
                    file_id = (
                        video_data_field.get("file_id")
                        or video_data_field.get("file")
                        or video_comp.get("file_id")
                        or video_comp.get("file")
                    )
                    ...
                    if not video_url and file_id:
                        try:
                            file_data = await call_onebot(
                                client, "get_file", file_id=file_id, file=file_id
                            )
                    ...
                    if not video_url and file_id and is_file_video:
                        try:
                            file_data = await call_onebot(
                                client, "download_file", url=file_id
                            )
                            file_payload = file_data.get("data", file_data)
                            video_url = file_payload.get("file") or file_payload.get(
                                "path"
                            )
                        except Exception as e:
                    ...
                    return self._format_error(
                        "获取视频信息", e, f"message_id={message_id}"
                    )
                ...
                try:
                    ...
                    if "b23.tv" in bilibili_input:
                        ...
                        if url_match:
                            real_input = await self._resolve_short_link(
                                url_match.group(1)
                            )
                    ...
                    bilibili_meta = await self._get_bilibili_video_data(
                        bvid, aid, p, qn
                    )
                    ...
                    logger.error(
                        f"Error getting bilibili info: {e}\n{traceback.format_exc()}"
                    )
                ...
                try:
                    ...
                    file_name = video_info.get(
                        "filename", f"video_element_{element_id}.mp4"
                    )
                    ...
                    logger.error(
                        f"Error getting video from browser element: {e}\n{traceback.format_exc()}"
                    )
            ...
            try:
                ...
                if video_url.startswith("base64://"):
                    ...
                    try:
                        ...
                        logger.info(
                            f"Video copied from local path: {decoded_path} -> {local_file_path}"
                        )
                    except Exception as e:
                        return self._format_error(
                            "复制本地视频文件", e, f"源路径: {video_url}"
                        )
                else:
                                ...
                                with open(local_file_path, "wb") as f:
                                    ...
                                    while True:
                                        ...
                                        if downloaded > size_limit_mb * 1024 * 1024:
                                            raise Exception(
                                                f"下载过程中超出大小限制 ({size_limit_mb}MB)"
                                            )
                ...
                logger.info(
                    f"Video downloaded successfully: {local_file_path} ({actual_size_mb:.2f}MB)"
                )
            ...
            try:
                ...
                logger.error(
                    f"Error processing with Gemini: {e}\n{traceback.format_exc()}"
                )
                return self._format_error(
                    "Gemini API 调用",
                    e,
                    f"模型: {model_id}, API地址: {api_url}, 上传方式: {upload_mode}",
                )
        finally:
            # 清理临时文件
    ...
    def _is_local_path(self, path: str) -> bool:
        ...
        if len(decoded_path) >= 2:
            # Windows 路径: C:\ 或 C:/
            if decoded_path[1] == ":" and (
                len(decoded_path) == 2 or decoded_path[2] in ("\\", "/")
            ):
                return True
    ...
    def _parse_generate_response(
        self, resp_text: str, model_id: str
    ...
    async def _process_with_file_api(
        ...
        logger.info(
            f"Using File API (Resumable Upload) to upload: {file_path} ({file_size_mb:.2f}MB, {mime_type})"
        )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    async with session.post(
                        ...
                        try:
                            ...
                            uploaded_file_name = upload_result.get("file", {}).get(
                                "name", ""
                            )
                            uploaded_file_uri = upload_result.get("file", {}).get(
                                "uri", ""
                            )
                            file_state = upload_result.get("file", {}).get("state", "")

                            logger.info(
                                f"File uploaded: name={uploaded_file_name}, uri={uploaded_file_uri}, state={file_state}"
                            )
                ...
                if uploaded_file_name:
                    ...
                    while total_waited < max_wait_time:
                        check_url = (
                            f"{api_base}/v1beta/{uploaded_file_name}?key={api_key}"
                        )

                        try:
                            async with session.get(
                                check_url, timeout=aiohttp.ClientTimeout(total=30)
                            ) as resp:
                                if resp.status == 200:
                                    ...
                                    logger.info(
                                        f"File state: {file_state} (waited {total_waited}s)"
                                    )
                ...
                # Step 3: 调用生成接口
                generate_url = (
                    f"{api_base}/v1beta/models/{model_id}:generateContent?key={api_key}"
                )
                ...
                logger.info(
                    f"Calling generateContent with file_uri: {uploaded_file_uri}"
                )
                    ...
                    async with session.post(
                        ...
                        if resp.status != 200:
                            try:
                                ...
                                error_code = error_json.get("error", {}).get(
                                    "code", resp.status
                                )
                            except:
                ...
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.delete(
                            ...
                            if resp.status == 200:
                                logger.info(
                                    f"Deleted uploaded file: {uploaded_file_name}"
                                )
                            else:
                                logger.debug(
                                    f"Failed to delete uploaded file: {resp.status}"
                                )
                except Exception as e:
                    logger.debug(f"Error deleting uploaded file: {e}")

    async def _process_with_inline_base64(
        self,
        api_base: str,
        api_key: str,
        model_id: str,
        ...
        try:
            async with aiohttp.ClientSession() as session:
                logger.info(
                    f"Sending request to Gemini API: {model_id}, timeout={timeout}s"
                )
    ...
    async def _get_video_from_browser_element(self, element_id: int) -> Dict:
        ...
        try:
            ...
            if result.get("isBlob"):
                return {
                    "error": "该视频使用了 blob URL，无法直接下载。请尝试使用其他方式获取视频，或查找视频的真实源地址。"
                }
    ...
    async def _get_bilibili_video_data(
        self, bvid: str, aid: str, p: int, qn: int
    ) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to get WBI keys: {e}, will try unsigned playurl"
                )
            ...
            async with session.get(
                play_url, params=signed_params, headers=headers
            ) as resp:
