...
class WakeTask:
    ...
    def format_display(self) -> str:
        ...
        if remaining > 0:
            if remaining >= 3600:
                remaining_str = (
                    f"{remaining // 3600}h{(remaining % 3600) // 60}m{remaining % 60}s"
                )
            elif remaining >= 60:
...
class WakeScheduler:
    ...
    async def create_task(
        ...
        async with self._lock:
            ...
            logger.info(
                f"Wake task created: {task_id} for session {session_id}, trigger at {task.trigger_time_str()}"
            )
            return task_id
    ...
    async def clear_tasks(self, session_id: Optional[str] = None) -> int:
        ...
        async with self._lock:
            if session_id:
                ...
                # 清空所有未触发的任务
                to_delete = [
                    tid for tid, task in self._tasks.items() if not task.triggered
                ]
            ...
            logger.info(
                f"Cleared {len(to_delete)} wake tasks"
    ...
    async def _trigger_task(self, task: WakeTask):
        ...
        logger.info(
            f"Triggering wake task: {task.task_id} for session {task.session_id}"
        )
        ...
        if self._wake_callback:
            try:
                ...
                logger.error(
                    f"Failed to execute wake callback for task {task.task_id}: {e}"
                )
        else:
