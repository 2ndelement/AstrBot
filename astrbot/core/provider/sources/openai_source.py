...
class ProviderOpenAIOfficial(Provider):
        ...
        for _attempt in range(1, _max_retries + 1):
            try:
                ...
                if _attempt < _max_retries:
                    _wait = 2 ** (_attempt - 1)
                    logger.warning(
                        f"[_query] 请求失败（第 {_attempt}/{_max_retries} 次），{_wait}s 后重试。原因：{_e}"
                    )
                    await asyncio.sleep(_wait)
                else:
                    logger.error(
                        f"[_query] 已重试 {_max_retries} 次仍失败，放弃。原因：{_e}"
                    )
                    raise _last_exc
    ...
    async def _parse_openai_completion(
        ...
        if llm_response.completion_text is None and not llm_response.tools_call_args:
            # If finish_reason is tool_calls but no tool matched, it may be an unknown tool.
            # Strategy: inject a tool-result error for each unknown call so the LLM sees
